 ...
 */
const postArbitrary = (availableTags: Tag[]) => fc.record({
  id: fc.uuid(),
  ...
  categoryId: fc.uuid(),
  authorId: fc.uuid(),
  tags: fc.subarray(availableTags, { minLength: 0, maxLength: Math.min(5, availableTags.length) }),
 ...
 */
const testScenarioArbitrary = fc.array(tagArbitrary, { minLength: 1, maxLength: 10 })
  .chain(tags => 
    fc.array(postArbitrary(tags), { minLength: 1, maxLength: 20 })
      .chain(posts => 
...
describe('Property: Related Posts Algorithm Correctness', () => {
  it('should always exclude the current post from results', () => {
    fc.assert(
      fc.property(
      ...
      fc.property(
        ...
        ({ currentPost, allPosts, limit }) => {
          ...
          if (related.length > 0 && currentPost.tags.length > 0) {
            ...
            const postsWithSharedTags = allPosts.filter(post => 
        ...
        fc.array(tagArbitrary, { minLength: 1, maxLength: 5 })
          .chain(tags =>
            fc.uuid().chain(categoryId =>
              fc.array(
                validDatePostArbitrary(tags, categoryId),
  ...
  it('should fallback to same category posts when no shared tags exist', () => {
    fc.assert(
      fc.property(
        fc.array(tagArbitrary, { minLength: 5, maxLength: 10 })
          .chain(tags => {
              ...
              fc.tuple(
                ...
                fc.array(
                  postArbitrary(tags).chain(post => 
                    fc.subarray(otherTags, { minLength: 0, maxLength: 3 })
                      .map(selectedTags => ({
                        ...post,
                        categoryId,
                        tags: selectedTags
    ...
    fc.assert(
      fc.property(
        ...
        ({ currentPost, allPosts, limit }) => {
          ...
          const currentTagIds = new Set(currentPost.tags.map(t => t.id));
          const hasSharedTagsOrCategory = allPosts.some(post => 
            post.id !== currentPost.id && (
              post.tags.some(tag => currentTagIds.has(tag.id)) ||
    ...
    fc.assert(
      fc.property(
        ...
        ({ currentPost, allPosts, limit }) => {
          ...
          const currentTagIds = new Set(currentPost.tags.map(t => t.id));
          const sharedTagCounts = related.map(post => 
            post.tags.filter(tag => currentTagIds.has(tag.id)).length
          );
  ...
  it('should handle limit of 0', () => {
    fc.assert(
      fc.property(
        fc.array(tagArbitrary, { minLength: 1, maxLength: 10 })
          .chain(tags => 
            fc.array(postArbitrary(tags), { minLength: 2, maxLength: 20 })
              .chain(posts => 
                fc.integer({ min: 0, max: posts.length - 1 })
                  .map(index => ({ currentPost: posts[index], allPosts: posts }))
              )
  ...
  it('should handle large limit (greater than available posts)', () => {
    fc.assert(
      fc.property(
        fc.array(tagArbitrary, { minLength: 1, maxLength: 10 })
          .chain(tags => 
            fc.array(postArbitrary(tags), { minLength: 2, maxLength: 10 })
              .chain(posts => 
                fc.integer({ min: 0, max: posts.length - 1 })
                  .map(index => ({ currentPost: posts[index], allPosts: posts }))
              )
