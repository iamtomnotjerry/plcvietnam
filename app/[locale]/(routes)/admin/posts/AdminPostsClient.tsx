    ...
    <div className="overflow-x-auto rounded-xl border border-border bg-card shadow-sm">
      <table className="w-full min-w-[640px] text-left text-sm">
        <thead className="border-b border-border bg-muted/40">
          <tr>
            <th className="px-4 py-3 font-medium">{t('postsTable.colTitle')}</th>
            <th className="px-4 py-3 font-medium">{t('postsTable.colSlug')}</th>
            <th className="px-4 py-3 font-medium">{t('postsTable.colStatus')}</th>
            <th className="px-4 py-3 font-medium">{t('postsTable.colUpdated')}</th>
            <th className="px-4 py-3 font-medium text-right">{t('postsTable.colActions')}</th>
          </tr>
        </thead>
        <tbody>
          {posts.length === 0 ? (
            ...
            posts.map((post) => (
              <tr key={post.id} className="border-b border-border/80 last:border-0">
                ...
                <td className="px-4 py-3">
                  ...
                  >
                    {(post.status ?? 'published') === 'draft'
                      ? t('postsTable.statusDraft')
                      : t('postsTable.statusPublished')}
                  </span>
                </td>
                <td className="px-4 py-3 text-muted-foreground">
                  {post.updatedAt.toLocaleDateString(intlLocale)}
                </td>
                <td className="px-4 py-3">
